...
const FollowingPage = async ({ params }: { params: { userId: string } }) => {
  const followings = await prismadb.follow.findMany({
    where: {
  ...
  const users = await prismadb.userSettings.findMany({
    where: {
      OR: followings.map((item) => ({ userId: item.following_id }))
    }
  });
    ...
    <div className="h-screen w-full p-2 overflow-y-auto no-scrollbar">
      <div className="text-white min-h-full w-full overflow-auto bg-bg-2 rounded-xl p-6 flex flex-col gap-8">
        <div className="flex flex-row items-center gap-4">
          ...
          <span className="text-white text-2xl font-semibold ">Following</span>
        </div>
