...
const getApiKey = async (): Promise<string> => {
  const result: CreateProjectKeyResponse = await (
    await fetch("/api/deepgram", { cache: "no-store" })
  ).json();
...
const DeepgramContextProvider = ({ children }: DeepgramContextInterface) => {
  ...
  const [ttsOptions, setTtsOptions] = useState<SpeakSchema | undefined>(defaultTtsOptions)
  const [sttOptions, setSttOptions] = useState<LiveSchema | undefined>(defaultSttsOptions)
