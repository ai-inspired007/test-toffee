...
const VoiceLibraryModal = ({
  ...
  isModal: boolean;
  toggle: Function;
  originalVoice: Voice | undefined;
  setVoice: Dispatch<React.SetStateAction<Voice | undefined>>;
}) => {
  type ItemType = {
    id: string;
    name: string;
  ...
  const [filteredVoiceList, setFilteredVoiceList] = useState<Voice[]>([]);
    ...
    const delayTimeoutId = setTimeout(() => {
      if (selectedItemIds?.length) {
        setFilteredVoiceList(
          voiceList?.filter(
            (voice) =>
              selectedItemIds?.includes(voice.id) &&
