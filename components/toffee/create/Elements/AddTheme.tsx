...
interface ThemesProps {
  ...
  setThemeFile: Dispatch<SetStateAction<File | null>>;
}
...
const AddTheme = ({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div
        className="w-full h-[282px] rounded-lg flex flex-col p-8 relative"
      ...
      >
        ...
        <div className="flex flex-row gap-3">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className="text-[#DEDFE4] bg-bg-3 px-4 py-3 text-sm" style={{ borderRadius: "0px 16px 16px 16px" }}>{"Hey there!"}</span>
          </div>
        </div>
        <div className="flex gap-3 flex-row-reverse">
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className="text-[#DEDFE4] bg-bg-3 px-4 py-3 text-sm" style={{ borderRadius: "0px 16px 16px 16px" }}>{"Hey, could you give me some suggestions?"}</span>
          </div>
        </div>
        <div className="flex flex-row gap-3">
          <Image src={"/characters/zero.png"} alt="" className="rounded-full w-8 h-8" width={32} height={32} />
          <div className="flex flex-col gap-2">
            ...
            <span className="text-[#DEDFE4] bg-bg-3 px-4 py-3 text-sm" style={{ borderRadius: "0px 16px 16px 16px" }}>{"Sure, here some some suggestions for you"}</span>
          </div>
        </div>
        </div>
      </div>
      ...
      <div className="flex flex-row gap-4 overflow-x-auto py-2">
        <label className="h-[152px] w-[200px] rounded-lg flex flex-col border border-white/10 items-center justify-center cursor-pointer flex-shrink-0">
          <RiImageAddLine className="text-icon-3 w-6 h-6 mb-4" />
          <span className="text-text-main">Add your photo</span>
          <input type="file" accept="image/*" className="hidden" onChange={handleFileChange} />
        </label>
        
        {themes.map((theme, index) => (
          <div key={index} className={`relative h-[152px] w-[200px] rounded-lg flex flex-col items-center justify-center cursor-pointer p-2 flex-shrink-0 ${theme.id === selected.id ? "border-[1px] border-solid border-[#BC7F44]" : "border border-white/10"}`}>
