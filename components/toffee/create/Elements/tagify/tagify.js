...
(function (global, factory) {
        ...
        if (e.target == this.DOM.scope) {
          ...
          return;
            ...
            if (timeDiffFocus > 500) {
              if (this.state.dropdown.visible) this.dropdown.hide();else if (_s.dropdown.enabled === 0 && _s.mode != 'mix') this.dropdown.show();
              return;
            }
          }
      ...
      onPaste(e) {
        ...
        this.settings.hooks.beforePaste(e, {
          ...
          if (result) {
            ...
            } else if (this.settings.pasteAsTags) this.addTags(result, true);else this.state.inputText = result;
          }
        }).catch(err => err);
      },
  ...
  Tagify.prototype = {
    ...
    parseMixTags(s) {
      ...
      s = s.split(mixTagsInterpolator[0]).map((s1, i) => {
        ...
        if (!maxTagsReached && s2.length > 1 && (!enforceWhitelist || this.isTagWhitelisted(tagData.value)) && !(!duplicates && this.isTagDuplicate(tagData.value))) {
          transformTag.call(this, tagData); // in case "tagTextProp" setting is set to other than "value" and this tag does not have this prop
    ...
    selectTag(tagElm, tagData) {
      ...
      this.input.set.call(this, tagData[this.settings.tagTextProp || 'value'], true); // place the caret at the end of the input, only if a dropdown option was selected (and not by manually typing another value and clicking "TAB")
        ...
        rootNode.childNodes.forEach(node => {
          if (node.nodeType == 1) {
            if (node.classList.contains(that.settings.classNames.tag) && that.tagData(node)) {
              if (that.tagData(node).__removed) return;else result += _interpolator[0] + JSON.stringify(omit(that.tagData(node), that.dataProps)) + _interpolator[1];
              return;
            }
