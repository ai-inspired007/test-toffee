...
export interface LinkProps {
  ...
  setLinks: Dispatch<SetStateAction<CandyLink[] | null>>;
}
...
const AddLink = ({ links, setLinks }: LinkProps) => {
  ...
  const [newLink, setNewLink] = useState<CandyLink>({ title: '', url: '', icon: '' });
  ...
  const handleDelete = (index: number) => {
    if (links) {
      ...
      setLinks(updatedLinks.length > 0 ? updatedLinks : null);
    }
    setEditingIndex(null); 
  };
      ...
      {(!links || (editingIndex !== null && editingIndex === links.length)) && (
        <div className='flex flex-col w-full items-center justify-center rounded-lg border-dashed border-[1px] py-5 border-white/20 cursor-pointer' onClick={() => setModal(true)}>
          <div className='flex flex-row gap-2'>
            <RiLinkM className='w-6 h-6 text-icon-3' />
            <span className='text-white text-sm font-medium'>{"Add your link"}</span>
          </div>
          <div className='text-sm text-text-tertiary'>{"Supported links: Youtube, Spotify"}</div>
        </div>
      )}
        ...
        <div className="bg-bg-2 w-[476px] rounded-xl flex flex-col p-6 gap-6">
          ...
          <div className='flex flex-col gap-1'>
            ...
            <input
              ...
              className="text-sm text-text-sub font-medium bg-transparent border border-white/10 focus:outline-none px-4 py-3 rounded-lg placeholder:text-text-tertiary placeholder:text-[13px]"
              placeholder="Link url"
            />
          </div>
          {/* TODO: Adding label */}
          <div
            className="w-full cursor-pointer bg-gradient-to-r from-[#C28851] to-[#B77536] rounded-full text-center text-white px-4 py-1.5 font-medium border border-white/20"
