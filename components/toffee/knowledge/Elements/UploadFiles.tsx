...
export const getFileIcon = (mimeType: any) => {
...
interface UploadFileProps {
  ...
  setOldFiles?: (newFiles: Partial<KnowledgeFile>[] | undefined) => void;
  files: CandyFile[] | null;
  setFiles: Dispatch<SetStateAction<CandyFile[] | null>>;
}

const UploadFiles = ({ files, setFiles, oldFiles, setOldFiles }: UploadFileProps) => {
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const [editingFileIndex, setEditingFileIndex] = useState<number | null>(null);
  const [newFileName, setNewFileName] = useState<string>('');
  const [oldFileIndex, setOldFileIndex] = useState<string | undefined>("");
  ...
  const handleOldEditConfirm = () => {
    if (oldFiles && setOldFiles) {
      const updatedFiles = oldFiles.map((file) =>
        file.id === oldFileIndex ? { ...file, name: newFileName } : file
      );
  ...
  return (
    <div className="flex flex-col w-full gap-8">
      {oldFiles && oldFiles.map((item, index) => (
        <div key={`old-${index}`} className="flex flex-row gap-3 w-full text-white items-center">
          <div className="flex items-center justify-center">
            {getFileIcon(item.type)}
          </div>
          <div className="flex flex-col gap-1 flex-grow">
            {oldFileIndex === item.id ? (
              <input
                type="text"
                value={newFileName}
                ...
                onKeyDown={(event) => {
                  if (event.key === 'Enter') {
                    handleOldEditConfirm();
                  }
                }}
                className="text-sm text-text-sub  font-medium bg-transparent border border-white/10 w-full rounded p-1"
              />
            ) : (
              <span className="text-sm text-text-sub  font-medium">{item.name}</span>
            )}
            <span className="text-xs text-text-tertiary ">{formatBytes(item.size ? item.size : 0)}</span>
          </div>
          <div className="flex flex-row text-[#777777] gap-6 ml-auto">
            {editingFileIndex === index ? (
              <MingcuteCheckLine className="h-6 w-6 cursor-pointer" onClick={() => handleOldEditConfirm()} />
            ) : (
              <>
                  <RiFileEditLine
                    className="h-6 w-6 cursor-pointer"
                    onClick={() => {
                      setNewFileName(item.name ? item.name : "");
                      setOldFileIndex(item.id)
                    }} />
                  <RiDeleteBin6Line className="h-6 w-6 cursor-pointer"
                    onClick={() => {
                      if (oldFiles && setOldFiles) {
                        const updatedFiles = oldFiles?.filter((oldFile) => oldFile.id !== item.id);
                        setOldFiles(updatedFiles);
                      }
                    }}
                  />
      ...
      <div onClick={() => setModal(true)} className="rounded-lg w-full flex flex-col gap-2 items-center justify-center border border-dashed border-white/20 p-6 cursor-pointer">
        <div className="flex flex-row gap-2 text-white">
          <RiFileUploadLine className="h-5 w-5 text-[#B1B1B1]" />
          <span className=" font-normal text-sm">Upload Files</span>
        </div>
        <span className="text-text-tertiary font-light text-sm">{"Supported formats PDF, PNG, MP4, DOC"}</span>
      </div>
      <Modal isOpen={modal} onClose={() => false} className="w-full flex-col flex justify-center items-center">
        <div className="bg-bg-2 w-[476px] rounded-xl flex flex-col p-6 gap-6">
          <div className="w-full flex flex-row items-center gap-2">
            <span className="font-normal text-white">Add your document</span>
            <Tooltip text="Add more labels to increase the effectiveness of your knowledge pack" className="bg-[#242424] text-text-tertiary px-4 py-2 rounded-md text-xs w-56 bottom-8 -left-20">
          ...
          {addings ? addings.map((file, index)=>(
            ...
            <div className="flex flex-row gap-2 text-white">
              <RiFileUploadLine className="h-5 w-5 text-[#B1B1B1]" />
              <span className=" font-normal text-sm">Add document</span>
            </div>
            <span className="text-text-tertiary font-light text-sm">{"Supported formats PDF, PNG, MP4, DOC"}</span>
          </div>}
          <input
          ...
          />
          {/* TODO: Adding label */}
          <div className="w-full cursor-pointer bg-gradient-to-r from-[#C28851] to-[#B77536] rounded-full text-center text-white px-4 py-1.5 font-normal text-sm border border-white/20" onClick={handleFileInput}>Add file</div>
        </div>
      </Modal>
    </div>
  );
