...
export interface CandyText {
  ...
  knowledgePackId: string | undefined;
}
...
const CreatePage: React.FC = () => {
  ...
  const validateStep = (step: number) => {
    if (step === 0) {
      ...
      // it's ok if they don't choose to upload anything
  ...
  return (
    <div className="h-screen w-full p-2 overflow-y-auto no-scrollbar">
      {loading && <Loading />}
      <ToastContainer />
      <div className="flex flex-col rounded-2xl bg-bg-2 w-full min-h-full items-center justify-start relative p-6 pt-4 pb-10">
          ...
          <div className="flex flex-row rounded-full bg-black p-0.5 text-white gap-0.5">
            {steps.map((item, index) =>
              <div key={index} className={`flex flex-row items-center ${index === step && "bg-[#BC7F44] rounded-full p-0.5"}`}>
                <div className={(index == step ? "text-white " : "text-text-tertiary ") + (index < step ? "bg-bg-3 rounded-full w-7 h-7 flex items-center justify-center font-light text-sm" : "w-6 h-6 flex items-center justify-center bg-black rounded-full font-light text-sm")}>{index + 1}</div>
                {index === step && <div className="text-xs px-2 font-light">{item}</div>}
              </div>
            )}
          </div>
        </div>
        <div className="no-scrollbar flex h-full w-full max-w-[720px] flex-col gap-6 overflow-x-visible">
          {step === 0 && <>
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs font-light text-text-tertiary">{"Name"}</span>
              <input
                type="text"
                className="w-full font-light text-[13px] text-text-sub p-1 bg-transparent border border-white/10 outline-none resize-none overflow-hidden rounded-lg px-4 py-3"
                value={name}
                onChange={(e) => setName(e.target.value)}
              />
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-xs font-light text-text-tertiary">{"Description"}</span>
              <textarea name="description" id="" className="w-full font-light text-[13px] text-text-sub p-1 bg-transparent border border-white/10 outline-none resize-none overflow-hidden rounded-lg px-4 py-3" rows={2} value={description} onChange={(e) => setDescription(e.target.value)}></textarea>
            </div>
            <SelectSharing options={sharingOptions} selectedOption={selectedOption} setSelectedOption={setSelectedOption} />
            <div className="w-full bg-gradient-to-r from-[#C28851] to-[#B77536] rounded-full px-4 py-1.5 text-center text-sm text-white font-normal border-[1px] border-solid border-white/20 cursor-pointer" onClick={() => handleNextStep(0)}>
              {"Save changes"}
            </div>
          </>}
          {step === 1 && <>
            <div>
              <div className="w-full text-start text-[32px] font-medium text-white mt-24 mb-2">
                Add your knowledge
              </div>
              <span className="text-sm text-text-tertiary font-light">
          ...
          {step === 2 && <>
            <div className="w-full text-center mt-12 flex flex-col">
              <span className="text-[32px] font-medium text-white mb-2">Add custom themes</span>
              <span className="text-sm font-light text-text-tertiary">You are able to set your custom theme to the chat background</span>
            </div>
            <AddTheme themes={themes} selected={selectedTheme} setSelectedTheme={setSelectedTheme} addTheme={addTheme} deleteTheme={deleteTheme} />
            <div className="grid grid-cols-2 gap-4 mt-3">
