...
export default function Login({ isAuth }: { isAuth: boolean }) {
  ...
  const handleCredentialAuth = async (e: React.FormEvent) => {
    ...
    if (response?.ok) {
      ...
      if (response.error.includes("no user")) {
        setErrorForEmail("Account with that email address doesn't exist.");
      } else if (response.error.includes("wrong credentials")) {
        setErrorForPassword("Invalid password.");
      } else {
      ...
      <div className="flex flex-col w-[520px] gap-2 rounded-md bg-bg-3 ">
        <div className="px-10 pt-10 bg-gradient-to-t from-[#BC7F4400] to-[#BC7F4433] rounded-md">
          <Image
            ...
            className="inline"
          />
          <h1 className="font-inter text-white text-xl font-medium leading-7 mt-10">Sign In</h1>
          <span className="font-inter text-[13px] font-light text-text-tertiary mt-2">Welcome to work full of different characters</span>
        </div>
        <div className="px-10 flex flex-col gap-6 pb-10 mt-4">
          <div className="bg-white cursor-pointer rounded-full px-4 py-1.5 w-full flex flex-row justify-center gap-1 items-center" onClick={handleGoogleAuth}>
            ...
            <span className="text-black font-medium text-center font-inter text-sm p-1">Sign in with Google</span>
          </div>
          ...
          <form className="flex flex-col" onSubmit={handleCredentialAuth}>
            <div className="flex flex-col gap-1">
              <label htmlFor="username" className="text-xs font-inter text-text-tertiary">Email</label>
              <input type="text" name="username" className={`px-4 py-3 rounded-md border bg-transparent text-[13px] focus:outline-none text-white autofill:bg-transparent ${errorForEmail?"border-[#DF1C41]":"border-white/10"}`} onChange={(e)=>setEmail(e.target.value)}/>
              {errorForEmail && (
            ...
            <div className="flex flex-col gap-1 mt-6">
              ...
              <input type="password" name="password" className={`px-4 py-3 rounded-md border bg-transparent text-[13px] focus:outline-none text-white ${errorForPassword?"border-[#DF1C41]":"border-white/10"}`} onChange={(e)=>setPassword(e.target.value)}/>
              {errorForPassword && (
                <span className="text-[#DF1C41] font-inter text-xs">{errorForPassword}</span>
              )}
            </div>
            <Link href="/forget-password" className="mt-2 text-[#BC7F44] font-inter text-xs font-medium cursor-pointer">Forget password?</Link>
            <button className="mt-6 bg-gradient-to-r from-[#C28851] via-[#B77536] to-[#B77536] text-white font-medium text-center cursor-pointer rounded-full px-4 py-2 w-full">
              Sign in
            </button>
            <Link href="/signup" className="text-text-tertiary font-inter text-xs cursor-pointer mt-6">{"Don't have an account?  "} <span className="mt-2 text-[#BC7F44] font-inter text-xs font-medium cursor-pointer">Sign Up</span></Link>
          </form>
        </div>
      </div>
    </div>
