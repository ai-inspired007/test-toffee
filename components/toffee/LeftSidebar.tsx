...
import { User2, UserRoundPlus, UserRoundIcon, SettingsIcon } from "lucide-react";
import { RiUserLine, RiSettings4Line, RiLogoutCircleRLine } from "react-icons/ri";
  ...
  const RecentList = () =>
    characters.length > 0 && user ? (
      ...
      <div className="mt-16 flex w-full flex-col items-center pt-0.5">
        ...
        <span className="mt-4 text-center  text-sm font-medium text-toffee-text-additional">
          {"No recent chats"}
        </span>
  ...
  const SubPanel = () => (
    <div className="flex h-full flex-grow flex-col  overflow-hidden rounded-lg bg-toffee-bg-2 px-2 py-3">
      ...
      {open && (
        <button
          type="button"
          className="leading-4.5 group relative inline-flex h-12 w-full shrink-0 items-center overflow-hidden rounded-lg bg-toffee-bg-3 px-4 text-sm font-medium text-white transition duration-200 hover:bg-[#5865f2] hover:text-white focus-visible:outline-none"
        >
          Join our Discord
          <svg
            width="73"
            ...
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="absolute inset-y-0 right-0 transition duration-200 hover:translate-x-2"
          >
            <path
      ...
      {user ? (
        <DropdownMenu.Root>
          <DropdownMenu.Trigger className="text-toffee-icon-additional outline-none data-[state=open]:text-toffee-icon-3">
            <div
              className={`mt-4 ${open ? "pt-[7px] pr-[58px] pb-[9px] pl-4" : ""} flex cursor-pointer flex-row items-center gap-x-3 rounded-md transition duration-200  ${open ? "px-4 py-2 hover:bg-toffee-bg-additional" : "justify-center hover:opacity-90"}`}
            >
          ...
          <DropdownMenu.Content
            className="z-50 w-[240px] overflow-hidden rounded-lg bg-toffee-bg-additional text-[14px] leading-[18px] font-medium text-text-additional"
            align={open ? "center" : "end"}
          >
            <DropdownMenu.Item
              className="w-full px-3 py-[6px] outline-none data-[highlighted]:bg-[#323232] data-[highlighted]:text-violet-100"
            >
              <Link href={"/profile"} className="flex w-full cursor-pointer select-none items-center gap-2">
                <RiUserLine className="w-6 h-6" />Profile
              </Link>
            </DropdownMenu.Item>
            <DropdownMenu.Item
              className="flex w-full px-3 py-[6px] outline-none data-[highlighted]:bg-[#323232] data-[highlighted]:text-violet-100"
            >
              <Link href={"/settings"} className="flex w-full cursor-pointer select-none items-center gap-2">
                <RiSettings4Line className="w-6 h-6" />Settings
              </Link>
            </DropdownMenu.Item>
            <DropdownMenu.Item
              className="flex w-full px-3 py-[6px] outline-none data-[highlighted]:bg-[#323232] data-[highlighted]:text-violet-100"
            >
              <div className="flex w-full cursor-pointer select-none items-center gap-2" onClick={() => signOut({ callbackUrl: '/' })}>
